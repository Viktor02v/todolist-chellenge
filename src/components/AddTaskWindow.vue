...
<template>
	<section  v-show="taskStore.toggleToAdd"
		...
		<div class="relative">
			<input @input="taskStore.handleContent" class="border pl-[40px] p-2 rounded-full border-none" type="text">
			<Edit :size="'30px'" class="p-1 absolute top-0 left-0 rounded-full border" />
		</div>
		<div class="flex items-center gap-2">
			<button @click="taskStore.toggleToAdd = false"
				class="w-[6rem] md:w-[8rem] p-2 bg-[#38a169] text-[1rem] font-bold text-white hover:bg-green-800 rounded-full"
				aria-label="Come Back To Home Page">Back</button>
			<button @click="taskStore.addTask"
				class="w-[6rem] md:w-[8rem] p-2 bg-[#38a169] text-[1rem] font-bold text-white hover:bg-green-800 rounded-full"
				aria-label="Create New Task">Create</button>
		</div>
	</section>
</template>
